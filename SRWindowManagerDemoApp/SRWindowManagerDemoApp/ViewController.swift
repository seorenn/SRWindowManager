...
class ViewController: NSViewController, NSOutlineViewDataSource, NSOutlineViewDelegate, NSTableViewDelegate {
    ...
    func refresh(_ reloadData: Bool = false) {
        self.items = SRWindowManager.applicationInfos.map { AppItem(application: $0) }
    ...
    func outlineView(_ outlineView: NSOutlineView, objectValueFor tableColumn: NSTableColumn?, byItem item: Any?) -> Any? {
        guard item != nil, let tableColumn = tableColumn else { return nil }

        if convertFromNSUserInterfaceItemIdentifier(tableColumn.identifier) == "Application", let app = item as? AppItem {
            return app.name
        } else if convertFromNSUserInterfaceItemIdentifier(tableColumn.identifier) == "Window", let window = item as? SRWindowInfo {
            return window.name
        }
